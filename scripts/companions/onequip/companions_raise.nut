this.companions_raise <- this.inherit("scripts/skills/skill", {
	...
	function spawnUndead( _user, _tile )
	{
		...
		if (e != null)
		{
			...
			{
				e.getItems().unequip(currentOffhand);
			}
			...
			{
				e.getItems().unequip(currentMainhand);
			}
			...
			local propertiesNew = {
				...
				Armor = [
					e.m.BaseProperties.Armor[0],
					e.m.BaseProperties.Armor[1]
				],
			...
			if (currentOffhand != null)
			{
				e.getItems().equip(currentOffhand);
			}

			if (currentMainhand != null)
			{
				e.getItems().equip(currentMainhand);
			}
			...
			if (this.m.Item.m.Quirks.len() != 0)
			{
				foreach( i, quirk in this.m.Item.m.Quirks )
				{
					...
					if (hasQuirk == null && getQuirk.m.ID != "quirk.good_boy")
					{
						e.m.Skills.add(this.new(quirk));
					}
				}
			}

			if (e.getFlags().get("zombie_minion"))
			{
				e.m.AIAgent = this.new("scripts/ai/tactical/agents/zombie_agent");
			}
			else if (e.getFlags().get("skeleton"))
			{
				if (this.isKindOf(e, "skeleton_priest"))
				{
