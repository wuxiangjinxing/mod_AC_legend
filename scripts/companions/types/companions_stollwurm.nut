this.companions_stollwurm <- this.inherit("scripts/entity/tactical/enemies/legend_stollwurm", {
	m = {
		Tail = null,
		Item = null,
		Name = "Stollwurm"
	...
	{
		this.legend_stollwurm.onDeath(_killer, _skill, _tile, _fatalityType);
	...
	{
		this.legend_stollwurm.onFactionChanged();
	...
	{
		this.legend_stollwurm.onActorKilled(_actor, _tile, _skill);
	...
	function onInit()
	{
		...
		if (this.m.Tail == null)
		{
			...
			if (spawnTile != null)
			{
				this.m.Tail = this.WeakTableRef(this.Tactical.spawnEntity("scripts/companions/types/companions_stollwurm_tail", spawnTile.Coords.X, spawnTile.Coords.Y, this.getID()));
