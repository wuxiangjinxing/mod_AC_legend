this.arena_contract <- this.inherit("scripts/contracts/contract", {
	...
	function setScaledDifficultyMult()
	{
		local addACtoStrength = false;
		local p = this.World.State.getPlayer().getStrength(addACtoStrength);
		...
		local d = this.Math.minf(5000.0, s);
		this.m.ScaledDifficultyMult = d * this.Const.Difficulty.EnemyMult[this.World.Assets.getCombatDifficulty()];
	}

	function getArenaReputationToPaymentMult()
	{
		local r = this.Math.minf(4000, this.Math.maxf(0.9, this.Math.pow(this.Math.maxf(0, 0.003 * this.World.Assets.getBusinessReputation() * 0.5 + this.m.ScaledDifficultyMult), 0.35)));
		return r * this.Const.Difficulty.PaymentMult[this.World.Assets.getEconomicDifficulty()];
	}

	function getArenaPaymentMult()
	{
		local repDiffMult = this.Math.pow(this.m.ScaledDifficultyMult, 0.5);
	...
	function setup()
	{
		...
		setScaledDifficultyMult();
		this.m.Payment.Pool = pay * this.getArenaPaymentMult() * this.getArenaReputationToPaymentMult();
	...
	function createScreens()
	{
		this.m.Screens.push({
			...
			function start()
			{
				...
				baseDifficulty = baseDifficulty * this.Contract.m.ScaledDifficultyMult;
